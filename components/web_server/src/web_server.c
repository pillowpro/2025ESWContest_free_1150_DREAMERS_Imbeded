...
"</div>"
"<script>"
"function submitConfig(){const ssid=document.getElementById('ssid').value;const password=document.getElementById('password').value;const provisioning_code=document.getElementById('provisioning_code').value;if(!ssid||!provisioning_code){showStatus('Please fill in SSID and Provisioning Code','error');return;}showStatus('Connecting...','');const btn=document.querySelector('button');btn.disabled=true;btn.textContent='Connecting...';fetch('/api/wifi-config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ssid:ssid,password:password,provisioning_code:provisioning_code})}).then(response=>response.json()).then(data=>{if(data.success){showStatus('Configuration saved! Device is connecting to WiFi...','success');setTimeout(()=>{window.location.reload();},3000);}else{showStatus('Error: '+data.message,'error');btn.disabled=false;btn.textContent='Connect';}}).catch(error=>{showStatus('Connection failed: '+error.message,'error');btn.disabled=false;btn.textContent='Connect';});}"
...
static esp_err_t wifi_config_post_handler(httpd_req_t *req)
{
    ...
    if (!json) {
        ...
        if (!cJSON_IsString(ssid_json) || !cJSON_IsString(provisioning_code_json)) {
            cJSON *success = cJSON_CreateBool(false);
            cJSON *message = cJSON_CreateString("Missing required fields: ssid, provisioning_code");
            cJSON_AddItemToObject(response_json, "success", success);
            cJSON_AddItemToObject(response_json, "message", message);
        } else {
